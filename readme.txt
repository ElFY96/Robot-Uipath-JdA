...
-----------------------------
          ...
          - Inside the 'Confi.xlsx' file User should put "Results_Document_File_Path" value and  "Filters_ExcelFile" value  full path.
          - The user will enter the full path of input data and output data in config file.
          - I order to initialize our robot with configuration that exists in config.xlsx file, we have two choices:
	1 - Set full config.xlsx file path manually by opening our robot in uipath and go to initialization, and then go to 'InitAllSettings'  and put the full path as a value of  in_ConfigFile.
	      It is not a good choice, because only devloper can do it, but user cannot do it.
	2 - Set 'Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + configFile'  as a value  of configFile   argument  to get config.xlsx full path.
	        		configFile is  "input data\Config.xlsx".
			Environment.GetFolderPath(Environment.SpecialFolder.Desktop) give us Desktop path.     

 
          - Avoid the 255 Character Limit in Excel Formulas :
                * 255 character limit is an excel feature that control  number of characters in a column. For our robot this is a problem. So in order to resolve this problem, I suggest to delete Characters over 
                     the limit and replace it with "...". (For getting the full details go to "copyContentToExcelFile" workflow.

          - If the link name contais   "   character, an error will occur, so to avoid that, I suggest to replace   "   character by  '  character  using replace method.

           -  To get the right data scrapping content and the correct excel file data, I've made some html elements changes to the site's interface manually, depending on inspect element, in order to get
                the correct data scraping, Because when we try to srape data normally, some data is confused.
